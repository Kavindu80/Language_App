...
export default function WordPuzzleScreenG1({ navigation, route }) {
  const { word = "GOAT", image = require("../../../../assets/items/goat.png") } = route?.params || {};
  ...
  const createPuzzleLetters = () => {
    ...
    const extraLetters = 'ABCDEFHIJKLMNPQRSUVWXYZ'
      .split('')
  ...
  const savePuzzleStatus = async (status) => {
    try {
      // Get current status
      const storedStatusJson = await AsyncStorage.getItem('gPuzzleStatus');
      ...
      // Save updated status
      await AsyncStorage.setItem('gPuzzleStatus', JSON.stringify(puzzleStatus));
    } catch (error) {
  ...
  const moveToNextPuzzle = () => {
    ...
    navigation.navigate('WordPuzzleScreenG2');
  };
