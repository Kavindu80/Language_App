...
export default function LetterDetailScreenG() {
  const navigation = useNavigation();
  const [isPlaying, setIsPlaying] = useState(false);
  const [audioType, setAudioType] = useState(null);
  
  // Hardcoded to display only "Gg"
  const letter = "Gg";
  ...
  return (
    ...
    >
      ...
      {/* Letter Title */}
      <Text style={styles.letterTitle}>G</Text>
      ...
      <TouchableOpacity 
        ...
        onPress={() => {
          // Navigate to the Words Starting with G Screen
          navigation.navigate("WordsStartingWithGScreen");
        }}
      >
