...
export default function WordPuzzleScreenH1({ navigation, route }) {
  const { word = "HAT", image = require("../../../../assets/items/hat.png") } = route?.params || {};
  ...
  const createPuzzleLetters = () => {
    ...
    const extraLetters = 'BCDEFGIJKLMNOPQRSUVWXYZ'
      .split('')
  ...
  const savePuzzleStatus = async (status) => {
    try {
      // Get current status
      const storedStatusJson = await AsyncStorage.getItem('hPuzzleStatus');
      ...
      // Save updated status
      await AsyncStorage.setItem('hPuzzleStatus', JSON.stringify(puzzleStatus));
    } catch (error) {
  ...
  const moveToNextPuzzle = () => {
    ...
    navigation.navigate('WordPuzzleScreenH2');
  };
