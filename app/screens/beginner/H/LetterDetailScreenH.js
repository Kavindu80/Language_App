...
export default function LetterDetailScreenH() {
  const navigation = useNavigation();
  const [isPlaying, setIsPlaying] = useState(false);
  const [audioType, setAudioType] = useState(null);
  
  // Hardcoded to display only "Hh"
  const letter = "Hh";
  ...
  return (
    ...
    >
      ...
      {/* Letter Title */}
      <Text style={styles.letterTitle}>H</Text>
      ...
      <TouchableOpacity 
        ...
        onPress={() => {
          // Navigate to the Words Starting with H Screen
          navigation.navigate("WordsStartingWithHScreen");
        }}
      >
