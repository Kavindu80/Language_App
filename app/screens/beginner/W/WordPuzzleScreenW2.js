...
export default function WordPuzzleScreenW2({ navigation, route }) {
  const { word = "WINDOW", image = require("../../../../assets/items/window.png") } = route?.params || {};
  ...
  const createPuzzleLetters = () => {
    ...
    const extraLetters = 'ABCEFGHJKLMPQRSTUVXYZ'
      .split('')
  ...
  const savePuzzleStatus = async (status) => {
    try {
      const storedStatusJson = await AsyncStorage.getItem('wPuzzleStatus');
      let puzzleStatus = storedStatusJson ? JSON.parse(storedStatusJson) : {};
      
      puzzleStatus[2] = status;
      
      await AsyncStorage.setItem('wPuzzleStatus', JSON.stringify(puzzleStatus));
    } catch (error) {
  ...
  const moveToNextPuzzle = () => {
    ...
    navigation.navigate('WordPuzzleScreenW3');
  };
