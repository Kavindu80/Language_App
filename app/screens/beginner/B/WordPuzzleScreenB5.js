...
export default function WordPuzzleScreen5({ navigation, route }) {
  const { word = "BIRD", image = require("../../../../assets/items/bird.png") } = route?.params || {};
  ...
  const savePuzzleStatus = async (status) => {
    try {
      // Get current status
      const storedStatusJson = await AsyncStorage.getItem('bPuzzleStatus');
      ...
      // Save updated status
      await AsyncStorage.setItem('bPuzzleStatus', JSON.stringify(puzzleStatus));
    } catch (error) {
  ...
  const moveToNextPuzzle = () => {
    ...
    navigation.navigate('VerbsScreenB');
  };
