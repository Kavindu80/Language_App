...
export default function WordPuzzleScreenK4({ navigation, route }) {
  const { word = "KANGAROO", image = require("../../../../assets/items/kangaroo.png") } = route?.params || {};
  ...
  const createPuzzleLetters = () => {
    ...
    const extraLetters = 'BCDEFHIJLMPQSTUVWXYZ'
      .split('')
  ...
  const savePuzzleStatus = async (status) => {
    try {
      const storedStatusJson = await AsyncStorage.getItem('kPuzzleStatus');
      let puzzleStatus = storedStatusJson ? JSON.parse(storedStatusJson) : {};
      
      puzzleStatus[4] = status;
      
      await AsyncStorage.setItem('kPuzzleStatus', JSON.stringify(puzzleStatus));
    } catch (error) {
  ...
  const moveToNextPuzzle = () => {
    ...
    navigation.navigate('WordPuzzleScreenK5');
  };
