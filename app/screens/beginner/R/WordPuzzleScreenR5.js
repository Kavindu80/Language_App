...
export default function WordPuzzleScreenR5({ navigation, route }) {
  const { word = "ROSE", image = require("../../../../assets/items/rose.png") } = route?.params || {};
  ...
  const createPuzzleLetters = () => {
    const extraLettersCount = Math.min(10 - word.length, 6);
    const extraLetters = 'ABCDFGHJKLMNPQTUVWXYZ'
      .split('')
  ...
  const savePuzzleStatus = async (status) => {
    try {
      const storedStatusJson = await AsyncStorage.getItem('rPuzzleStatus');
      let puzzleStatus = storedStatusJson ? JSON.parse(storedStatusJson) : {};
      
      puzzleStatus[5] = status;
      
      await AsyncStorage.setItem('rPuzzleStatus', JSON.stringify(puzzleStatus));
    } catch (error) {
  ...
  const moveToNextPuzzle = () => {
    ...
    // Navigate to the alphabet selection screen or another appropriate screen
    navigation.navigate('VerbsScreenR');
  };
