...
export default function LetterDetailScreenV() {
  const navigation = useNavigation();
  const [isPlaying, setIsPlaying] = useState(false);
  const [audioType, setAudioType] = useState(null);
  
  // Hardcoded to display only "Vv"
  const letter = "Vv";
  ...
  return (
    ...
    >
      ...
      {/* Letter Title */}
      <Text style={styles.letterTitle}>V</Text>
      ...
      <TouchableOpacity 
        ...
        onPress={() => {
          // Navigate to the Words Starting with V Screen
          navigation.navigate("WordsStartingWithVScreen");
        }}
      >
