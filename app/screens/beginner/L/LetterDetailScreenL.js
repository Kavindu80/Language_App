...
export default function LetterDetailScreenL() {
  const navigation = useNavigation();
  const [isPlaying, setIsPlaying] = useState(false);
  const [audioType, setAudioType] = useState(null);
  
  // Hardcoded to display only "Ll"
  const letter = "Ll";
  ...
  return (
    ...
    >
      ...
      {/* Letter Title */}
      <Text style={styles.letterTitle}>L</Text>
      ...
      <TouchableOpacity 
        ...
        onPress={() => {
          // Navigate to the Words Starting with L Screen
          navigation.navigate("WordsStartingWithLScreen");
        }}
      >
