...
export default function WordPuzzleScreenM5({ navigation, route }) {
  const { word = "MAP", image = require("../../../../assets/items/map.png") } = route?.params || {};
  ...
  const createPuzzleLetters = () => {
    ...
    const extraLetters = 'BCDEFGHIJKLNOQRSTUVWXYZ'
      .split('')
  ...
  const savePuzzleStatus = async (status) => {
    try {
      const storedStatusJson = await AsyncStorage.getItem('mPuzzleStatus');
      let puzzleStatus = storedStatusJson ? JSON.parse(storedStatusJson) : {};
      
      puzzleStatus[5] = status;
      
      await AsyncStorage.setItem('mPuzzleStatus', JSON.stringify(puzzleStatus));
    } catch (error) {
  ...
  const moveToNextPuzzle = () => {
    ...
    // Navigate back to the alphabet screen or to a completion screen
    navigation.navigate('VerbsScreenM');
  };
