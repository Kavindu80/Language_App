...
export default function AlphabetScreen() {
  ...
  const playLetterSound = async (letter) => {
    ...
    try {
      setIsPlaying(true);
      // Extract just the first character (the uppercase letter)
      const upperCaseLetter = letter.charAt(0);
      await SpeechHelper.speak(`Letter ${upperCaseLetter}`, {
  ...
  const handleLetterPress = (letter) => {
    ...
    // Navigate to the appropriate screen after a small delay
    setTimeout(() => {
    ...
    }, 300); // Short delay to allow the sound to start playing
  };
  ...
  return (
    ...
    >
      ...
      {/* Instruction Text */}
      <Text style={styles.instruction}>Click the letter to start</Text>
