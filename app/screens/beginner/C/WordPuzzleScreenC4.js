...
export default function WordPuzzleScreenC4({ navigation, route }) {
  const { word = "COW", image = require("../../../../assets/items/cow.png") } = route?.params || {};
  ...
  const createPuzzleLetters = () => {
    ...
    const extraLetters = 'ABDEFGHIJKLMNPQRSTUVXYZ'
      .split('')
  ...
  const savePuzzleStatus = async (status) => {
    try {
      // Get current status
      const storedStatusJson = await AsyncStorage.getItem('cPuzzleStatus');
      let puzzleStatus = storedStatusJson ? JSON.parse(storedStatusJson) : {};
      
      // Update status for this puzzle (id 4)
      puzzleStatus[4] = status;
      
      // Save updated status
      await AsyncStorage.setItem('cPuzzleStatus', JSON.stringify(puzzleStatus));
    } catch (error) {
  ...
  const moveToNextPuzzle = () => {
    ...
    navigation.navigate('WordPuzzleScreenC5');
  };
