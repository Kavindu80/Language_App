...
export default function WordPuzzleScreenN4({ navigation, route }) {
  const { word = "NET", image = require("../../../../assets/items/net.png") } = route?.params || {};
  ...
  const createPuzzleLetters = () => {
    ...
    const extraLetters = 'ABCDFGHIKLMOPQRSUVWXYZ'
      .split('')
  ...
  const savePuzzleStatus = async (status) => {
    try {
      const storedStatusJson = await AsyncStorage.getItem('nPuzzleStatus');
      let puzzleStatus = storedStatusJson ? JSON.parse(storedStatusJson) : {};
      
      puzzleStatus[4] = status;
      
      await AsyncStorage.setItem('nPuzzleStatus', JSON.stringify(puzzleStatus));
    } catch (error) {
  ...
  const moveToNextPuzzle = () => {
    ...
    navigation.navigate('WordPuzzleScreenN5');
  };
