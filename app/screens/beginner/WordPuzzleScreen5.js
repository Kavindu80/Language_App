...
export default function WordPuzzleScreen5({ navigation, route }) {
  const { word = "ANT", image = require("../../../assets/items/ant.png") } = route?.params || {};
  ...
  // Create puzzle letters with word and some extra random letters
  const createPuzzleLetters = () => {
    const extraLettersCount = Math.min(15 - word.length, 6);
    const extraLetters = 'BCDEFGHIJKLMOPQRSUVWXYZ'
      .split('')
  ...
  const checkAnswer = () => {
    ...
    setIsCorrect(submittedWord === word);
  ...
  // Navigate to future screen
  const navigateToFuture = () => {
    // Navigate to your future screen
    navigation.navigate('VerbsScreenA');
  };
  
  // Adaptive image size based on screen width and available height
  const screenHeight = height - 230; // Account for header, padding, etc.
      ...
      <View style={styles.container}>
        ...
        <View style={styles.mainContent}>
          ...
          <View style={[styles.imageContainer, { width: imageSize, height: imageSize, borderRadius: imageSize / 2 }]}>
            <Image 
              source={typeof image === 'number' ? image : require("../../../assets/items/ant.png")} 
          ...
          <Text style={styles.helpText}>
            {placedTiles.includes(null) ? 
              "Tap letters to make a word." : 
              "Tap a letter to take it out."
            }
          </Text>
          
          {/* Letter Tiles to Select */}
        ...
        {/* Feedback and Navigation - Position Absolute to avoid content shift */}
        {showFeedback && (
          <View style={styles.feedbackContainer}>
            ...
            <View style={styles.actionButtonsContainer}>
              {/* Future Button - Replaces Next Puzzle */}
              {isCorrect && (
                <TouchableOpacity 
                  style={[styles.actionButton, styles.futureButton]}
                  onPress={navigateToFuture}
                  activeOpacity={0.7}
                >
                  <FontAwesome5 
                    name="rocket" 
                  ...
                  />
                  <Text style={styles.actionButtonText}>Future</Text>
                </TouchableOpacity>
              )}
...
const styles = StyleSheet.create({
  ...
  // Feedback and actions with fixed positioning to avoid content shifting
  feedbackContainer: {
    position: 'absolute',
    bottom: 0,
    left: 0,
  ...
  futureButton: { backgroundColor: '#9C27B0' },
