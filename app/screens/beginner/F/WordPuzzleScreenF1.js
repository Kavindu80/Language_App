...
export default function WordPuzzleScreenF1({ navigation, route }) {
  const { word = "FISH", image = require("../../../../assets/items/fish.png") } = route?.params || {};
  ...
  const createPuzzleLetters = () => {
    ...
    const extraLetters = 'ABCDEGHJKLMNOPQRSTUVWXYZ'
      .split('')
  ...
  const savePuzzleStatus = async (status) => {
    try {
      // Get current status
      const storedStatusJson = await AsyncStorage.getItem('fPuzzleStatus');
      let puzzleStatus = storedStatusJson ? JSON.parse(storedStatusJson) : {};
      
      // Update status for this puzzle (id 1)
      puzzleStatus[1] = status;
      
      // Save updated status
      await AsyncStorage.setItem('fPuzzleStatus', JSON.stringify(puzzleStatus));
    } catch (error) {
  ...
  const moveToNextPuzzle = () => {
    ...
    navigation.navigate('WordPuzzleScreenF2');
  };
