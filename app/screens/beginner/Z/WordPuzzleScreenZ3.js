...
export default function WordPuzzleScreenZ3({ navigation, route }) {
  const { word = "ZIPPER", image = require("../../../../assets/items/zipper.png") } = route?.params || {};
  ...
  const createPuzzleLetters = () => {
    ...
    const extraLetters = 'ABCDEFGHJKLMNQSTUVWXY'
      .split('')
  ...
  const savePuzzleStatus = async (status) => {
    try {
      const storedStatusJson = await AsyncStorage.getItem('zPuzzleStatus');
      let puzzleStatus = storedStatusJson ? JSON.parse(storedStatusJson) : {};
      
      puzzleStatus[3] = status;
      
      await AsyncStorage.setItem('zPuzzleStatus', JSON.stringify(puzzleStatus));
    } catch (error) {
  ...
  const moveToNextPuzzle = () => {
    ...
    navigation.navigate('WordPuzzleScreenZ4');
  };
