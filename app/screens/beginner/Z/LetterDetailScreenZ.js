...
export default function LetterDetailScreenZ() {
  const navigation = useNavigation();
  const [isPlaying, setIsPlaying] = useState(false);
  const [audioType, setAudioType] = useState(null);
  
  // Hardcoded to display only "Zz"
  const letter = "Zz";
  ...
  return (
    ...
    >
      ...
      {/* Letter Title */}
      <Text style={styles.letterTitle}>Z</Text>
      ...
      <TouchableOpacity 
        ...
        onPress={() => {
          // Navigate to the Words Starting with Z Screen
          navigation.navigate("WordsStartingWithZScreen");
        }}
      >
