...
export default function WordPuzzleScreenD2({ navigation, route }) {
  const { word = "DUCK", image = require("../../../../assets/items/duck.png") } = route?.params || {};
  ...
  const createPuzzleLetters = () => {
    ...
    const extraLetters = 'ABCEFGHIJKLMNOPQRSTUVWXYZ'
      .split('')
  ...
  const savePuzzleStatus = async (status) => {
    try {
      const storedStatusJson = await AsyncStorage.getItem('dPuzzleStatus');
      let puzzleStatus = storedStatusJson ? JSON.parse(storedStatusJson) : {};
      
      puzzleStatus[2] = status;
      
      await AsyncStorage.setItem('dPuzzleStatus', JSON.stringify(puzzleStatus));
    } catch (error) {
  ...
  const moveToNextPuzzle = () => {
    ...
    navigation.navigate('WordPuzzleScreenD3');
  };
